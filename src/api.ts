...
export const getGiftCards = async (skip = 0, limit = 100, activeOnly = true) => {
  try {
...
export const uploadGiftCardImage = async (file: FormData) => {
  try {
    const response = await api.post(`/api/v1/gift-cards/transactions/upload-card-image`, file, {
      headers: {
...
export const getGiftCardTransactionById = async (transactionId: string) => {
  try {
    const response = await api.get(`/api/v1/gift-cards/transactions/${transactionId}`);
    return response.data;
  } catch (error) {
...
export const updateGiftCardTransaction = async (transactionId: string, data: { status?: string, notes?: string }) => {
  try {
    const response = await api.put(`/api/v1/gift-cards/transactions/${transactionId}`, data);
    return response.data;
  } catch (error) {
...
export const getChatMessages = async (skip: number = 0, limit: number = 100) => {
  try {
...
export const createChatMessage = async (message: string, attachment_url?: string) => {
  try {
...
export const getVtuTransactions = async (skip: number = 0, limit: number = 100) => {
  try {
