...
const cryptoAssets: CryptoAsset[] = [
  { value: 'BTC', label: 'Bitcoin (BTC)', icon: '₿', color: '#F7931A', rate: 26500000 }, // 1 BTC = ₦26,500,000
  { value: 'ETH', label: 'Ethereum (ETH)', icon: 'Ξ', color: '#627EEA', rate: 1750000 }, // 1 ETH = ₦1,750,000
  { value: 'USDT', label: 'Tether (USDT)', icon: '₮', color: '#26A17B', rate: 1575 }, // 1 USDT = ₦1,575
];
...
const valueRanges = {
  BTC: [
    ...
    { value: 'medium', label: '$1,000 - $10,000', minAmount: 0.03, maxAmount: 0.3 },
    { value: 'high', label: '$10,000+', minAmount: 0.3, maxAmount: 100 },
  ],
  ETH: [
    { value: 'low', label: '$10 - $1,000', minAmount: 0.005, maxAmount: 0.5 },
    { value: 'medium', label: '$1,000 - $10,000', minAmount: 0.5, maxAmount: 5 },
    { value: 'high', label: '$10,000+', minAmount: 5, maxAmount: 1000 },
  ],
  USDT: [
    { value: 'low', label: '$10 - $1,000', minAmount: 10, maxAmount: 1000 },
    { value: 'medium', label: '$1,000 - $10,000', minAmount: 1000, maxAmount: 10000 },
    { value: 'high', label: '$10,000+', minAmount: 10000, maxAmount: 1000000 },
  ],
};

const sellSteps = ['Select Asset', 'Enter Details', 'Review', 'Send Crypto', 'Upload Proof'];
const buySteps = ['Select Asset', 'Payment Method', 'Enter Amount', 'Review', 'Complete Payment'];
...
const CryptoPage: React.FC = () => {
  ...
  const handleSellChange = (prop: keyof PaymentDetails) => (
    ...
    setSellDetails((prev) => {
      ...
      if (prop === 'amount' || prop === 'asset') {
        const selectedAsset = cryptoAssets.find((asset) => asset.value === newDetails.asset);
        if (selectedAsset && newDetails.amount) {
          newDetails.totalNaira = parseFloat(newDetails.amount) * selectedAsset.rate;
        }
      }
  ...
  const handleBuyChange = (prop: string) => (
    event: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>
  ) => {
    const value = event.target.type === 'checkbox' ? (event.target as HTMLInputElement).checked : event.target.value;
    ...
    setBuyDetails((prev) => {
      ...
      if (prop === 'totalNaira' || prop === 'asset') {
        ...
        if (selectedAsset && newDetails.totalNaira) {
          newDetails.amount = (parseFloat(newDetails.totalNaira.toString()) / selectedAsset.rate).toFixed(8);
        }
      }
  ...
  const renderChatDialog = () => (
    <Dialog open={chatOpen} onClose={handleChatToggle} maxWidth="sm" fullWidth>
      ...
      <DialogContent>
        <Box sx={{ p: 2, height: '300px', backgroundColor: '#f5f5f5', borderRadius: 1, mb: 2 }}>
              ...
              >
                {cryptoAssets.map((asset) => (
                  <MenuItem key={asset.value} value={asset.value}>
                    <Box display="flex" alignItems="center">
                      <Avatar sx={{ bgcolor: asset.color, width: 24, height: 24, mr: 1, fontSize: '0.8rem' }}>
                        {asset.icon}
                      </Avatar>
                      {asset.label}
                    </Box>
      ...
      case 1: // Enter Details
        return (
          <Box sx={{ p: 3 }}>
                ...
                InputProps={{
                  startAdornment: (
                    <InputAdornment position="start">
                      {cryptoAssets.find(a => a.value === sellDetails.asset)?.icon || ''}
                    </InputAdornment>
                  ),
                }}
              />
            ...
            {sellDetails.amount && (
              <Box sx={{ mb: 3, p: 2, bgcolor: 'rgba(107, 77, 255, 0.05)', borderRadius: 1 }}>
                ...
                <Typography variant="caption" color="text.secondary">
                  Rate: ₦{cryptoAssets.find(a => a.value === sellDetails.asset)?.rate.toLocaleString('en-NG')} per {sellDetails.asset}
                </Typography>
              </Box>
            )}
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between', mt: 3 }}>
              ...
              <GradientButton
                onClick={handleNext}
                disabled={!sellDetails.amount || !sellDetails.agreedToTerms || !sellDetails.confirmedDetails}
              >
                ...
                <Stack spacing={2}>
                  <Box display="flex" justifyContent="space-between">
                    ...
                    <Typography variant="body1" fontWeight="medium">
                      {cryptoAssets.find(a => a.value === sellDetails.asset)?.label}
                    </Typography>
                  </Box>
                  ...
                  <Box display="flex" justifyContent="space-between">
                    ...
                    <Typography variant="body1" fontWeight="medium">
                      {networks[sellDetails.asset]?.find(n => n.value === sellDetails.network)?.label}
                    </Typography>
                  </Box>
                  ...
                  <Box display="flex" justifyContent="space-between">
                    <Typography variant="body2" color="text.secondary">Amount to Sell</Typography>
                    <Typography variant="body1" fontWeight="medium">
                  ...
                  {sellDetails.comments && (
                    <>
                      ...
                      <Box>
                        ...
                        <Typography variant="body2">{sellDetails.comments}</Typography>
                      </Box>
                    </>
                  )}
                </Stack>
        ...
        return (
          <Box sx={{ p: 3 }}>
            ...
            <Alert severity="info" sx={{ mb: 3 }}>
              Please send exactly {sellDetails.amount} {sellDetails.asset} to the address below.
            </Alert>
            ...
            <StyledCard sx={{ mb: 3 }}>
              <CardContent>
                ...
                <Alert severity="warning">
                  Only send {sellDetails.asset} through the {sellDetails.network} network. Sending any other asset or using another network may result in permanent loss of funds.
                </Alert>
              </CardContent>
            </StyledCard>
      ...
      case 4: // Upload Proof
        return (
          <Box sx={{ p: 3 }}>
            ...
            <Alert severity="info" sx={{ mb: 3 }}>
              Please upload a screenshot of your transaction for verification purposes.
            </Alert>
            ...
            >
              {previewImage ? (
                ...
                <Box>
                  <CloudUploadIcon sx={{ fontSize: 60, color: '#6B4DFF', mb: 2 }} />
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
              ...
              <GradientButton 
                ...
                startIcon={processingTransaction ? <CircularProgress size={20} color="inherit" /> : null}
              >
            ...
            {transactionCompleted && (
              <Box sx={{ mt: 3, p: 3, bgcolor: '#f0f9eb', borderRadius: 2, textAlign: 'center' }}>
                <CheckCircleIcon sx={{ fontSize: 60, color: '#52c41a', mb: 2 }} />
                <Typography variant="h6" gutterBottom>
                  Transaction Submitted!
                </Typography>
                <Typography variant="body1">
                  Your sale request is being processed and funds will be credited to your account shortly. Thanks for trusting us!
                </Typography>
                <Button
                  variant="outlined"
                  sx={{ mt: 2 }}
  ...
  {paymentMethods.map((method) => (
    <Box key={method.id}>
      <Paper
        ...
        sx={{
          p: 2,
          border: buyDetails.paymentMethod === method.id ? '2px solid #6B4DFF' : '1px solid #eee',
          borderRadius: 2,
          cursor: 'pointer',
          transition: 'all 0.3s ease',
          '&:hover': {
        ...
        onClick={() => {
          setBuyDetails({ ...buyDetails, paymentMethod: method.id });
        }}
      >
        <Box display="flex" alignItems="center">
          ...
          <Box>
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between', mt: 3 }}>
              ...
              <GradientButton onClick={handleNext} disabled={!buyDetails.paymentMethod}>
                Next <ArrowForwardIcon sx={{ ml: 1 }} />
              </GradientButton>
            </Box>
          </Box>
          ...
          <Box sx={{ p: 3 }}>
            ...
            {buyDetails.totalNaira > 0 && (
              <Box sx={{ mb: 3, p: 2, bgcolor: 'rgba(107, 77, 255, 0.05)', borderRadius: 1 }}>
                ...
                <Typography variant="caption" color="text.secondary">
                  Rate: ₦{cryptoAssets.find(a => a.value === buyDetails.asset)?.rate.toLocaleString('en-NG')} per {buyDetails.asset}
                </Typography>
              </Box>
            )}
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between', mt: 3 }}>
              ...
              <GradientButton
                onClick={handleNext}
                disabled={!buyDetails.totalNaira || !buyDetails.agreedToTerms || !buyDetails.confirmedDetails}
              >
                Review <ArrowForwardIcon sx={{ ml: 1 }} />
              </GradientButton>
            </Box>
              ...
              <CardContent>
                <Stack spacing={2}>
                  <Box display="flex" justifyContent="space-between">
                    <Typography variant="body2" color="text.secondary">Asset</Typography>
                    <Typography variant="body1" fontWeight="medium">
                      {cryptoAssets.find(a => a.value === buyDetails.asset)?.label}
                    </Typography>
                  </Box>
                  ...
                  <Box display="flex" justifyContent="space-between">
                    <Typography variant="body2" color="text.secondary">Network</Typography>
                    <Typography variant="body1" fontWeight="medium">
                      {networks[buyDetails.asset]?.find(n => n.value === buyDetails.network)?.label}
                    </Typography>
                  </Box>
                  ...
                  <Box display="flex" justifyContent="space-between">
                    ...
                    <Typography variant="body1" fontWeight="medium">
                      {paymentMethods.find(m => m.id === buyDetails.paymentMethod)?.label}
                    </Typography>
                  </Box>
                  ...
                  <Box display="flex" justifyContent="space-between">
                    ...
                    <Typography variant="body1" fontWeight="medium">
                      ₦{parseFloat(buyDetails.totalNaira.toString()).toLocaleString('en-NG')}
                    </Typography>
                  </Box>
                  ...
                  {buyDetails.comments && (
                    <>
                      ...
                      <Box>
                        <Typography variant="body2" color="text.secondary">Comments</Typography>
                        <Typography variant="body2">{buyDetails.comments}</Typography>
                      </Box>
                    </>
                  )}
                </Stack>
        ...
        return (
          <Box sx={{ p: 3 }}>
            ...
            {buyDetails.paymentMethod === 'bank' ? (
              <>
                <Alert severity="info" sx={{ mb: 3 }}>
                  Please transfer ₦{parseFloat(buyDetails.totalNaira.toString()).toLocaleString('en-NG')} to the account details below.
                </Alert>
                  ...
                  <CardContent>
                    <Stack spacing={2}>
                      <Box display="flex" justifyContent="space-between">
                        ...
                        <Typography variant="body1" fontWeight="medium">
                          {paymentMethods.find(m => m.id === 'bank')?.accountDetails?.bankName}
                        </Typography>
                      </Box>
                      ...
                      <Box display="flex" justifyContent="space-between">
                        ...
                        <Typography variant="body1" fontWeight="medium">
                          {paymentMethods.find(m => m.id === 'bank')?.accountDetails?.accountNumber}
                        </Typography>
                      </Box>
                      ...
                      <Box display="flex" justifyContent="space-between">
                        ...
                        <Typography variant="body1" fontWeight="medium">
                          {paymentMethods.find(m => m.id === 'bank')?.accountDetails?.accountName}
                        </Typography>
                      </Box>
                    </Stack>
                  </CardContent>
              ...
              <StyledCard sx={{ mb: 3 }}>
                <CardContent>
                  ...
                  <FormControl fullWidth sx={{ mb: 2 }}>
                    <TextField label="Card Number" variant="outlined" placeholder="1234 5678 9012 3456" />
                  </FormControl>
...
>
  <TextField label="Expiry Date" variant="outlined" placeholder="MM/YY" fullWidth />
  <TextField label="CVV" variant="outlined" placeholder="123" fullWidth />
</Box>
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
              ...
              <GradientButton 
                onClick={completeTransaction}
                disabled={buyDetails.paymentMethod === 'bank' && !previewImage || processingTransaction}
                startIcon={processingTransaction ? <CircularProgress size={20} color="inherit" /> : null}
              >
            ...
            {transactionCompleted && (
              <Box sx={{ mt: 3, p: 3, bgcolor: '#f0f9eb', borderRadius: 2, textAlign: 'center' }}>
                ...
                <Typography variant="body1">
                  Your purchase is being processed and crypto will be sent to your wallet shortly. Thanks for trading with us!
                </Typography>
                <Button
                  variant="outlined"
                  sx={{ mt: 2 }}
      ...
      <MainContainer>
        ...
        <Paper sx={{ borderRadius: 3, overflow: 'hidden', mb: 4 }}>
          <Box sx={{ borderBottom: 1, borderColor: 'divider' }}>
            ...
            >
              <Tab icon={<SellIcon />} label="Sell Crypto" iconPosition="start" />
              <Tab icon={<ShoppingCartIcon />} label="Buy Crypto" iconPosition="start" />
            </Tabs>
          </Box>
...
>
  {cryptoAssets.map((asset) => (
    <Box key={String(asset.value)}>
      <StyledCard>
        <CardContent>
          <Box display="flex" alignItems="center" justifyContent="space-between" sx={{ mb: 2 }}>
            <Box display="flex" alignItems="center">
              <Avatar sx={{ bgcolor: asset.color, mr: 1 }}>{asset.icon}</Avatar>
          ...
          <Typography variant="body2" color="text.secondary">
            {Math.random() > 0.5 ? 'Up' : 'Down'} {(Math.random() * 5).toFixed(2)}% today
          </Typography>
        </CardContent>
      </StyledCard>
    </Box>
      ...
      <StyledBottomNav value={activeTab} onChange={(_, newValue) => setActiveTab(newValue)}>
