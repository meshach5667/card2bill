...
const HomePage: React.FC = () => {
  ...
  const balance: number = 250000.00;
  const savingsBalance: number = 50000.00;
  const cryptoBalance: number = 25000.00;
  ...
  const stats: StatsItem[] = [
    { id: 1, label: 'Total Savings', amount: savingsBalance, icon: '💰', trend: '+12.5%' },
    { id: 2, label: 'Crypto Value', amount: cryptoBalance, icon: '₿', trend: '+5.2%' },
  ...
  const recentTransactions: Transaction[] = [
    ...
    { id: 2, type: 'Received', amount: 5000, date: 'Yesterday', icon: '💰' },
    { id: 3, type: 'Electricity', amount: -2000, date: 'Mar 15', icon: '⚡' },
  ];
      ...
      >
        <Box sx={modalStyle}>
          ...
          <FormControl fullWidth sx={{ mb: 3 }}>
            <Typography variant="subtitle2" sx={{ mb: 1 }}>Select Bank</Typography>
            <Select
              defaultValue=""
              displayEmpty
              sx={{ borderRadius: 2 }}
            >
              <MenuItem value="" disabled>Select your bank</MenuItem>
          ...
          <Typography variant="caption" sx={{ color: 'text.secondary', display: 'block', mb: 4 }}>
            Withdrawal processing time: 5-10 minutes. Fee: ₦100 for amounts over ₦10,000.
          </Typography>
  ...
  {billOptions[currentBillType].map((provider) => (
    <Box 
      key={provider.id}
      sx={{ 
        width: { xs: 'calc(50% - 8px)', sm: 'calc(33.333% - 10.667px)' }
      }}
    >
      <Button
        fullWidth
